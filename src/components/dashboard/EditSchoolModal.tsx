...
import { SchoolForm } from './AddSchoolModal';
  ...
  useEffect(() => {
    if (school) {
      setForm({
        ...
        phone: school.phone || '',
        email: school.email || '',
        subscription_type: school.subscription_type || 'flex',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-8 relative">
        ...
        <h2 className="text-2xl font-bold mb-4">Modifier l'école</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end mt-6 space-x-3">
            ...
            >
              {loading ? 'Modification en cours...' : "Modifier l'école"}
            </button>
          </div>
        </form>
      </div>
